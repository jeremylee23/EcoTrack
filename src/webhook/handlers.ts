...
import { classifyIntent, generateRagResponse } from "../services/rag.service.js";
import {
  getUserPrefs,
  setUserPrefs,
  upsertFavorite,
  ...
  shortenAddress,
  favoriteDisplayName,
} from "../services/prefs.service.js";
import { isSamePlace } from "../utils/place-sync.util.js";
...
async function handleTextMessage(
  ...
  if (favSave) {
    await replyFavoritesMenu(
      userId,
      replyToken,
      withQuickReply(buildTextMessage("不用這樣打字。請直接點選單就好。"))
    );
    return;
  }
...
async function replyWithProcessingError(event: Event): Promise<void> {
  try {
    const replyToken =
      event.type === "follow"
...
export async function handleLineEvent(event: Event): Promise<void> {
  try {
    ...
    if (event.type === "follow") {
      ...
      if (!followEvent.replyToken) return;
      await handleFollowEvent(userId, followEvent.replyToken);
      return;
    }
    ...
    if (messageEvent.message.type === "location") {
      await handleLocationMessage(
        userId,
        replyToken,
        messageEvent.message as LocationMessageContent
      );
      return;
    }
    ...
    if (messageEvent.message.type === "text") {
      await handleTextMessage(
        userId,
        replyToken,
        messageEvent.message as TextMessageContent
      );
    }
  } catch (err) {
    console.error("[Webhook] Handler error:", err);
    await replyWithProcessingError(event);
  }
}